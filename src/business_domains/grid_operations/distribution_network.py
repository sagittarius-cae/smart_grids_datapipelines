...
class DistributionNetworkEntity:
     def __init__(self) -> None:
              self.spark = get_spark_session()
              self.io_client = DataLakeIO(self.spark)

     def create_rawdata(self) -> None:
         df =  self.io_client.read_csv("08_distribution_networks.csv") 
         self.io_client.write_delta_table(self.spark, df, "local.db_raw.distribution_networks") 
